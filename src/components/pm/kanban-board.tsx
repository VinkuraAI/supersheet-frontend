...
export function KanbanBoard({ workspaceId, onCreateClick, onEditTask, tasks, onTaskMove }: KanbanBoardProps) {
                ...
                <div key={col.id} className="w-[280px] flex flex-col h-full rounded-sm bg-slate-100/50">
                  <div className="p-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-slate-400 font-medium">
                        {filteredTasks.filter(t => (t.data?.status || 'todo') === col.id).length}
                      </span>
                    </div>
                  ...
                  <Droppable droppableId={col.id}>
                    {(provided: any, snapshot: any) => (
                      ...
                      >
                        {filteredTasks
                          .filter(t => (t.data?.status || 'todo') === col.id)
                          .map((task, index) => (
